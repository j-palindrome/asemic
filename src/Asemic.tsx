...
export function AsemicCanvas<T extends SettingsInput>({
  children,
  className,
  ...
  settings: T
  children: ((builder: SceneBuilder<T>) => ReactNode)[]
}) {
      ...
      <Canvas
        onClick={ev => {
          if (children.length > 0) {
            setScene((scene + 1) % children.length)
          }
          if (ev.shiftKey) {
        ...
        {started && frameloop === 'always' && (audio || !useAudio) && (
          <AsemicContext.Provider value={{ audio }}>
            <Scene {...{ settings, scene }}>{children}</Scene>
...
function Scene<T extends SettingsInput>({
  children,
  scene,
  settings
}: {
  ...
  const builder = useAsemic<T>(settings)
  return children[scene](builder)
}
