...
export class DefaultFont extends AsemicFont {
  constructor() {
    super({
      ...
      i: (p: Parser) =>
        p.to('+0.5,0').crv('0,0 +0,-1').cir('0,-1.5 .25,.25').tra('+-.5,0'),
      j: (p: Parser) =>
        p
          .to('+.5,0')
          .crv('0,-1')
      ...
      m: (p: Parser) =>
        p
          .crv('0,0 0,-1')
          .crv('<1 +.25,-.2 +.25,.2 .5,0')
          .crv('<.65 +.25,-.2 +.25,.2 1,0'),
      n: p =>
        p.crv('0,0 0,-1').tri('0,-.9 @0,1 -0.2', { add: true }).crv('1,0'),
      o: p => p.cir('.5,-.5 .5,.5'),
      p: p => p.crv('0,1 @-1/4,2').pen('+0,0 @1/4,1 -1,.2'),
      q: p => p.crv('1,1 @-1/4,2').pen('+0,0 @1/4,1 1,.2'),
      r: p => p.crv('0,0 @-1/4,1').tri('0,-.9 @0,1 -0.2'),
      s: p => p.squ('.7,-1 .5,-.5 .3', { add: true }).crv('.7,-.4 .7,0 0,0'),
      ...
      S: p => p.squ('1,-1.7 .5,-1 .5', { add: true }).squ('+0,0 0,0 -1'),
      ...
      '\\^': p => p.to('+0,2 >'),
      '\\.': p => p.to('+1.25,0'),
      '1': p => p.crv('.5,0 +0,-2').crv('+0,0 0,-1.5'),
      '2': p => p.crvs('1,0 0,0', '0,0 1,-.2 1,-1 0,-1 0,-.8'),
      '3': p => p.hex('0,-.7 .8,-.5 .5,.2').hex('+0,0 0,0 .5 .2'),
      '4': p => p.crvs('0,-2 0,-1 +0,0 +1,0', '.7,0 +0,-1.5'),
      '5': p => p.hex('0,-.2 0,-1 1,.2').crvs('+0,0 0,-2 +0,0 1,-2'),
      '6': p => p.cir('.5,-.5 .5,.5').crv('+-1,0 0,-2 1,-2'),
      '7': p => p.crvs('0,0 1,-2', '+0,0 0,-2'),
      '8': p => p.cir('.5,-.5 .5,.5').cir('.5,-1.4 .4,.4'),
      '9': p => p.cir('.5,-1.5 .5,.5').crv('+0,0 0,0 1,0'),
      '(': p => p.tri('.5,0 .5,-2 -.3').to('+-.4,0'),
      ')': p => p.to('+-.5,0').tri('.5,0 .5,-2 .3')
    })
  }
}
