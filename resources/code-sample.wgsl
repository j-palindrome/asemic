...
@group(1) @binding(0)
var<uniform> st: vec2<f32>;
@group(1) @binding(1)
var<uniform> time: f32;
@group(1) @binding(2)
var<uniform> speed: f32;
@group(1) @binding(2)
var<uniform> blending: f32;
...
fn vertexMain(@builtin(vertex_index) vertex_index: u32) -> VertexOutput {
  ...
  var uvs = array<vec2<f32>, 6>(vec2<f32>(0.0, 1.0), vec2<f32>(1.0, 1.0), vec2<f32>(0.0, 0.0), vec2<f32>(1.0, 1.0), vec2<f32>(1.0, 0.0), vec2<f32>(0.0, 0.0));
...
fn noise(_st: vec2<f32>, scale: f32, offset: f32) -> vec4<f32> {
